...
export const actions = {
  ...
  async publishPost({ commit }, { payload }) {
    ...
    delete putData.user
    await this.$axios.$put(`/users/${user.uid}/posts.json`, [
      ...(user.posts || []),
      putData
    ])
    commit('addPost', { post })
  },
  async addLikeToPost({ commit }, { uid, post }) {
    post.likes.push({
  ...
  async removeLikeToPost({ commit }, { uid, post }) {
    post.likes = post.likes.filter(like => like.user_uid !== uid) || []
    const newPost = await this.$axios.$put(`/posts/${post.id}.json`, post)
    commit('updatePost', { post: newPost })
  }
}
